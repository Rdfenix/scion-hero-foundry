...
import Wheel from "../assets/png/combat-wheel.png";
import Token from "../assets/png/time_token.png";
import CombatWheel from "./components/CombatWheel";
...
export class ScionCombatWheel {
  ...
  static async openDialog(stepDirection) {
    ...
    const dialog = new foundry.applications.api.DialogV2({
      ...
      content: `<div class="combat-dialog-react"></div>`,
      buttons: [
        {
          action: "select",
          label: "Select",
          ...
          callback: async () => {
            ...
            if (stepDirection === "next") {
              await this.moveToken(current + Number.parseInt(values.tick));
              reactRoot?.unmount();
            } else {
              await this.moveToken(current - Number.parseInt(values.tick));
              reactRoot?.unmount();
        ...
        },
        {
          ...
          callback: () => reactRoot?.unmount(),
        },
      ],
    });

    dialog.render(true).then(() => {
      const mountPoint = dialog.element?.querySelector(".combat-dialog-react");

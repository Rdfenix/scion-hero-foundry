...
import { knackSchema, boonSchema } from "./actor-base-default";
import { stripHTMLAndFormatTable } from "../utils/utils";
...
const prepareBoonList = async (actor, boonName) => {
  try {
    ...
    const allPowers = boonsList
      .flatMap((b) => b.purviews)
      .flatMap((p) => p.boons);
    ...
    if (!newList.some((b) => b.name === game.i18n.localize(foundBoon.name))) {
      newList.push({
        ...
        description: stripHTMLAndFormatTable(
          game.i18n.localize(foundBoon.description),
        ),
...
const prepareBirthrightList = async (actor, boonName, index) => {
  try {
    ...
    if (birthrights[index]) {
      if (
        !birthrights[index].boons.some(
          (b) => b.name === game.i18n.localize(foundBoon.name),
        )
      ) {
...
export async function _onDrop(event, actor) {
  ...
  if (!rawData) {
    console.warn("Drop ignorado: nenhum dado text/plain encontrado.");
    return;
  }
  ...
  // 4. Lógica de Processamento de Boons
    ...
    if (dropTarget === "birth-boons-list" && index !== undefined) {
      const updatedBirthrights = await prepareBirthrightList(
        actor,
        data.entry,
        index,
      );
      if (updatedBirthrights)
        updates["system.birthrights"] = updatedBirthrights;
    }
  }

...
export async function createPuviewsJournal() {
  ...
  for (const purview of purviewList) {
    ...
    const content = await foundry.applications.handlebars.renderTemplate(templatePath, { purview });
...
export async function createKnacksJournal() {
  ...
  for (const knackItem of knackList) {
    ...
    const content = await foundry.applications.handlebars.renderTemplate(templatePath, { knackItem });
